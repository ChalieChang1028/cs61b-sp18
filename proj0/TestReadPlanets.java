...
public class TestReadPlanets {
    ...
    private static String checkReadPlanets() {
        ...
        for (Planet p : actualOutput) {
            if ("images/earth.gif".equals(p.imgFileName)) {
                foundEarth = true;
            ...
            } else if ("images/mars.gif".equals(p.imgFileName)) {
                foundMars = true;
            } else if ("images/mercury.gif".equals(p.imgFileName)) {
                foundMercury = true;
                if (!doubleEquals(p.yyPos, 0, 0.01)) {
                    System.out.println("Advice: Your Mercury doesn't have the right yyPos!");
                    randomChecksOkay = false;
                }
            } else if ("images/sun.gif".equals(p.imgFileName)) {
                foundSun = true;
            } else if ("images/venus.gif".equals(p.imgFileName)) {
                foundVenus = true;
